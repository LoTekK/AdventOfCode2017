...
class Virus:
    ...
    def update(self):
        if self.grid[self.pos[1]][self.pos[0]]:
            self.turn(1)
        else:
            self.turn(-1)
            self.infected += 1
        self.grid[self.pos[1]][self.pos[0]] = not self.grid[self.pos[1]][self.pos[0]]
        self.move()


with utils.get_input(22) as f:
    input = [[(c == '#') for c in i.strip()] for i in f.readlines()]
    # input = ['.........', '.........', '.........', '.....#...', '...#.....', '.........', '.........', '.........', '.........']
    # input = [[(c == '#') for c in i] for i in input]
    v = Virus(input)
